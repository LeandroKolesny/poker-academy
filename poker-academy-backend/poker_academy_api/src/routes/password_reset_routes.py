...
def forgot_password():
    ...
    try:
        ...
        # Enviar email com nome do usuário
        email_sent = EmailService.send_password_reset_email(user.email, token, user.name)
        if not email_sent:
            return jsonify({"success": False, "message": "Erro ao enviar email"}), 500
        
        print(f"✅ Token de reset gerado para {user.name} ({user.email}): {token}")
...
def validate_reset_token():
    ...
    try:
        ...
        # Decodificar token se necessário (URL decode)
        import urllib.parse
...
def reset_password():
    ...
    try:
        ...
        print(f"✅ Senha redefinida para usuário: {user.name} ({user.email})")
